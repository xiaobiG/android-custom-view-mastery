...
`.github/workflows/deploy-pages.yml` 仅在 `main` 的 push 或手动触发时运行。GitHub Actions 环境会自动把 VitePress `base` 设置为：
...
本地构建不会设置该 base，仍以 `http://127.0.0.1:4000/` 为根路径阅读。仓库保持私有，但 GitHub Pages 站点可能公开访问；推送前仍应执行隐私扫描。
